...
async def async_setup_platform(hass, config, async_add_entities, discovery_info=None):
    ...
    sensor_icq2 = EGDPowerDataSensor(hass, client_id, client_secret, ean, days, "ICQ2", "mdi:transmission-tower-export")
    sensor_isq2 = EGDPowerDataSensor(hass, client_id, client_secret, ean, days, "ISQ2", "mdi:transmission-tower-import")

    async_add_entities([sensor_icq2, sensor_isq2])

    async def handle_event(event):
        _LOGGER.info(f"---Event received for {sensor_icq2.name} and {sensor_isq2.name}: {event.data}---")
        token = await sensor_icq2._get_access_token()
        if token:
            await sensor_icq2._update_state(token)
            await sensor_isq2._update_state(token)
            sensor_icq2._attributes["last_updated"] = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            sensor_isq2._attributes["last_updated"] = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            sensor_icq2.async_write_ha_state()
            sensor_isq2.async_write_ha_state()
        else:
...
class EGDPowerDataSensor(Entity):
    ...
    async def _get_access_token(self):
        async with aiohttp.ClientSession() as session:
            # FIX: OAuth2 token endpoint vyžaduje application/x-www-form-urlencoded
            # data= posílá form-encoded (správně), json= by poslalo JSON (špatně → 400)
            data = {
            ...
            _LOGGER.info(f"---Getting access token at {TOKEN_URL}---")
            async with session.post(TOKEN_URL, data=data) as response:
                if response.status < 400:
                    ...
                    _LOGGER.error(f"Error retrieving access token: {response.status} - {response_text}")
                    return None
    ...
    async def _update_state(self, token):
        ...
        # FIX: přidán pageStart=1, který API vyžaduje (chyběl → pravděpodobná příčina 400)
        params = {
            'ean': self.ean,
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(DATA_URL, headers=headers, params=params) as response:
                if response.status == 200:
                    ...
                    response_text = await response.text()
                    _LOGGER.error(f"Error retrieving data: {response.status} - {response_text}")
                    self._state = "Error"
